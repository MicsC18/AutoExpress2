...
async function AsignarRolEmpleado() {
...
function agregarUsuario() {
    const btnAgregarEmpleado = document.querySelectorAll('.btnAgregarEmpleado');
    btnAgregarEmpleado.forEach((boton) => {
        boton.addEventListener('click', function (e) {

            const idSucursalAgregar=document.getElementById('idSucursal');
            let ValorIdSucursal=e.target.getAttribute('data-id');

            idSucursalAgregar.value=ValorIdSucursal;
            console.log('id de la sucursal:',ValorIdSucursal )
            ...
            // Aquí asignamos el evento de cambio al select de roles
            selectRolUsuario.addEventListener('change', function () {
                ...
                // Hacer la petición para verificar el rol seleccionado
                fetch('../../Controlador/GestionSucursales/VerificarRol.php', {
                    ...
                    .then(response => {
                        ...
                        return response.text(); // Cambiar de .json() a .text() para ver el contenido crudo
                    })
                    .then(data => {
                        try {
                            const jsonData = JSON.parse(data); // Intentar parsear el texto como JSON
                            // Si el rol requiere una sucursal, habilitar los campos
